...
export default function Home() {
  ...
  const [lines, setLines] = useState([1]);
    ...
    const handleCursorMove = () => {
      ...
      const currentLine = Math.floor((cursorTop - editorTop + scrollTop) / lineHeight) + 1;
      if(currentLine > Math.max(...lines)) setLines(Array.from({ length: currentLine }, (_, i) => i + 1));
    ...
    const updateLineNumbers = () => {
      ...
      const totalLines = Math.ceil(editorRef.current.scrollHeight / lineHeight);
  ...
  return (
    <div className="flex">
     <div
      ...
      className="flex-1 p-4 outline-none max-h-screen h-full text-sm/6.5"
      ...
      <div className=" bg-amber-100 h-screen flex flex-col">
        <span className="p-4 flex items-center hover:bg-amber-200/30 text-2xl text-amber-950"><i className="fa-light fa-files"/></span>
        <span className="p-4 flex items-center hover:bg-amber-200/30 text-2xl text-slate-700"><i className="fa-light fa-gear"/></span>
      </div>
      </div>
  );
}
